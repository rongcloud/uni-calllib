...
export default class RCIMIWEngine {
     ...
     * @returns
     */
    copyFileToUniDocPath(filePath: string): Promise<string>;
    /**
     * 注册原生自定义普通消息
     ...
     */
    getLocalMessagesByMessageTypes(type: RCIMIWConversationType, targetId: string, channelId: string, messageTypes: RCIMIWMessageType[], sentTime: number, order: RCIMIWTimeOrder, count: number, callback: IRCIMIWGetMessagesCallback): Promise<number>;
    /**
     * 加载第一条未读消息。
     * @param type      会话类型
     * @param targetId  会话 ID
     ...
     * 代表当前接口调用操作失败，不会触发接口回调，详细错误参考错误码
     */
    searchMessagesByMessageTypes(type: RCIMIWConversationType, targetId: string, channelId: string, messageTypes: RCIMIWMessageType[], keyword: string, startTime: number, count: number, callback: IRCIMIWSearchMessagesByMessageTypesCallback): Promise<number>;
    /**
     * 根据关键字搜索会话。
     * @param conversationTypes 会话类型集合
     * @param channelId         频道 ID，仅支持超级群使用，其他会话类型传 null 即可。
     ...
     */
    getAppSettings(): Promise<RCIMIWAppSettings | number>;
    /**
     * 请求拉取流式消息
     * @param params   请求参数
     * @param callback 操作回调
     ...
     */
    requestStreamMessageContent(params: RCIMIWStreamMessageRequestParams, callback: IRCIMIWOperationCallback): Promise<number>;
    /**
     * 发送某个会话中的消息已读回执（V5）。
     ...
     */
    sendReadReceiptResponseV5(type: RCIMIWConversationType, targetId: string, channelId: string, messageUIds: string[], callback: IRCIMIWSendReadReceiptResponseV5Callback): Promise<number>;
    /**
     * 批量获取消息已读信息（V5）。
     * @param type        会话类型
     * @param targetId    会话目标 ID
     ...
     * @return 当次接口操作的状态码。0 代表调用成功，非 0 代表失败
     */
    getMessageReadReceiptInfoV5(type: RCIMIWConversationType, targetId: string, channelId: string, messageUIds: string[], callback: IRCIMIWGetMessageReadReceiptInfoV5Callback): Promise<number>;
    /**
     * 批量获取消息已读信息（V5，按消息标识）。
     ...
     * @return 当次接口操作的状态码。0 代表调用成功，非 0 代表失败
     */
    getMessageReadReceiptInfoV5ByIdentifiers(identifiers: RCIMIWMessageIdentifier[], callback: IRCIMIWGetMessageReadReceiptInfoV5Callback): Promise<number>;
    /**
     * 分页获取消息已读成员列表（V5）。
     * @param type       会话类型
     * @param targetId   会话目标 ID
     ...
     * @return 当次接口操作的状态码。0 代表调用成功，非 0 代表失败
     */
    getMessagesReadReceiptUsersByPageV5(type: RCIMIWConversationType, targetId: string, channelId: string, messageUId: string, option: RCIMIWReadReceiptUsersOption, callback: IRCIMIWGetMessagesReadReceiptUsersByPageV5Callback): Promise<number>;
    /**
     * 批量获取用户指定消息是否已读状态（V5）。
     * @param type       会话类型
     * @param targetId   会话目标 ID
     ...
     * @return 当次接口操作的状态码。0 代表调用成功，非 0 代表失败
     */
    getMessagesReadReceiptByUsersV5(type: RCIMIWConversationType, targetId: string, channelId: string, messageUId: string, userIds: string[], callback: IRCIMIWGetMessagesReadReceiptByUsersV5Callback): Promise<number>;
    /**
     * 收到消息的监听
     */
     ...
     * 流式消息请求初始化回调
     */
    setOnStreamMessageRequestInitListener(callback?: (res: OnStreamMessageRequestInitResult) => void): void;
    /**
     * 流式消息增量数据回调
     */
    setOnStreamMessageRequestDataListener(callback?: (res: OnStreamMessageRequestDataResult) => void): void;
    /**
     * 流式消息请求结束回调
     */
    setOnStreamMessageRequestCompleteListener(callback?: (res: OnStreamMessageRequestCompleteResult) => void): void;
    /**
     ...
     * 已读回执 V5 收到回调
     */
    setOnMessageReadReceiptV5ReceivedListener(callback?: (res: OnMessageReadReceiptV5ReceivedResult) => void): void;
}
